...
func borrow2N<NC1: ~Copyable, NC2: ~Copyable, Result>(
  ...
  _ nc2: borrowing NC2,
  n: Int,
  _ cb: (Int, borrowing NC1, borrowing NC2) throws -> Result
) rethrows -> [Result] {
  try (0..<n).map { i in
    try cb(i, nc1, nc2)
  }
...
func borrow2N<NC1: ~Copyable, NC2: ~Copyable, Result>(
  ...
  _ nc2: borrowing NC2,
  n: Int,
  group: inout ThrowingTaskGroup<Result, any Error>,
  _ cb: @escaping @Sendable (Int, borrowing NC1, borrowing NC2) async throws -> Result
) {
  ...
  for i in (0..<n) {
    groupPtr.value.pointee.addTask {
