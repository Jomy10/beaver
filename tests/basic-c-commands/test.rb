class BasicCCommand < Minitest::Test
  ...
  def test_cache
    Dir.chdir(__dir__) do
      ...
      assert_equal 0, $?.exitstatus
      assert_equal "", `#{windows? ? "set \"OUT=out\" &&" : "OUT=out"} ruby make.rb build`
      assert_equal "Building object\nBuilding object\nLinking\n", `#{windows? ? "set \"OUT=out\" &&" : "OUT=out"} ruby make.rb build -f`
      assert_equal 0, $?.exitstatus
    end
  end
  ...
  def test_run
    Dir.chdir(__dir__) do
      ...
      assert_equal 0, $?.exitstatus
      assert_equal "Hello world!", `#{windows? ? "set \"OUT=out\" &&" : "OUT=out"} ruby make.rb run`
      assert_equal 0, $?.exitstatus
    end
  end
